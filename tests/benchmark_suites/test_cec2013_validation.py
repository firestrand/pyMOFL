"""
Validation tests for CEC 2013 benchmark suite against golden datasets.
"""
...
SUITE_JSON = "src/pyMOFL/constants/cec/2013/cec2013_suite.json"
DATA_DIR = "src/pyMOFL/constants/cec/2013"
...
# (func_id, dim) -> reason; for failures that only affect certain dimensions
XFAIL_FUNC_DIMS: dict[tuple[int, int], str] = {
    (
        8,
        10,
    ): "Ackley: float64 amplification in asy transform (x^8 exponents on rotated values ~200 amplify ULP errors)",
    (
        8,
        30,
    ): "Ackley: float64 amplification in asy transform (x^8 exponents on rotated values ~200 amplify ULP errors)",
...
# Composition functions F21-F28 have deeply nested C-code-specific behaviors
# (buffer aliasing in asyfunc, hardcoded schwefel internals, etc.) that cannot
# be fully reproduced via external transform pipeline without monolithic reimplementation.
for _fid in range(21, 29):
    XFAIL_FUNCS[_fid] = (
        "CEC 2013 composition: C-code buffer aliasing and hardcoded schwefel "
        "internals not fully reproducible via modular transform pipeline"
    )


class TestCEC2013Validation:
    """Test all CEC 2013 functions against golden data."""

    @pytest.mark.parametrize("func_id", list(range(1, 29)))
    ...
    def test_function(self, factory, suite_config, func_id, dim):
        ...
        if (func_id, dim) in XFAIL_FUNC_DIMS:
            pytest.xfail(XFAIL_FUNC_DIMS[(func_id, dim)])

        cases = load_golden_cases("CEC2013", func_id, dim, GOLDEN_ROOT)
        if not cases:
...
class TestCEC2013Construction:
    """Test that all CEC 2013 functions can be constructed."""
