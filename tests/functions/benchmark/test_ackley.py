...
from pyMOFL.functions import AckleyFunction
...
class TestAckleyFunction:
    ...
    def test_initialization(self):
        ...
        custom_init_bounds = Bounds(
            low=np.array([-5, -5, -5]), high=np.array([5, 5, 5]), mode=BoundModeEnum.INITIALIZATION
        )
    ...
    # #         shifted_func = ShiftWrapper(inner_function=func, shift=shift)
    ...
    def test_bounds_respect(self):
        ...
        custom_init_bounds = Bounds(
            low=np.array([-1, -1]), high=np.array([1, 1]), mode=BoundModeEnum.INITIALIZATION
        )
        custom_oper_bounds = Bounds(
            low=np.array([-1, -1]), high=np.array([1, 1]), mode=BoundModeEnum.OPERATIONAL
        )
        func = AckleyFunction(
            dimension=2,
            initialization_bounds=custom_init_bounds,
