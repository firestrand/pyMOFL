...
from pyMOFL.functions import Schwefel_1_2, Schwefel_2_6


class TestSchwefel_1_2:
    """Tests for the Schwefel's Problem 1.2 function."""

    def test_initialization(self):
        """Test initialization with default and custom parameters."""
        # Default initialization
        func = Schwefel_1_2(dimension=3)
        ...
        custom_oper_bounds = Bounds(
            low=np.array([-5, -5, -5]), high=np.array([5, 5, 5]), mode=BoundModeEnum.OPERATIONAL
        )
        func = Schwefel_1_2(
            dimension=3,
    ...
    def test_evaluate_batch(self):
        ...
        expected = np.array(
            [func.evaluate(points[0]), func.evaluate(points[1]), func.evaluate(points[2])]
        )
...
class TestSchwefel_2_6:
    ...
    def test_initialization(self):
        ...
        func = Schwefel_2_6(dimension=dim, A=A, B=B, optimum_point=optimum_point)
        assert func.dimension == 3
        assert isinstance(func.initialization_bounds, Bounds)
        assert isinstance(func.operational_bounds, Bounds)
        np.testing.assert_allclose(func.initialization_bounds.low, [-100, -100, -100])
        ...
        assert func.optimum_point is not None
        ...
        B = np.dot(A, optimum_point)
        func = Schwefel_2_6(dimension=3, A=A, B=B, optimum_point=optimum_point)
        assert np.array_equal(func.A, A)
        assert func.optimum_point is not None
        assert np.array_equal(func.optimum_point, optimum_point)
        assert np.array_equal(
            func.B, np.ones(3)
        )  # B = A·optimum_point = optimum_point for identity A
    ...
    def test_evaluate_batch(self):
        ...
        B = np.dot(A, optimum_point)
        func = Schwefel_2_6(dimension=dim, A=A, B=B, optimum_point=optimum_point)
        points = np.array([optimum_point, np.array([0.0, 0.0, 0.0]), np.array([2.0, 2.0, 2.0])])
        expected = np.array(
            [func.evaluate(points[0]), func.evaluate(points[1]), func.evaluate(points[2])]
        )
        results = func.evaluate_batch(points)
