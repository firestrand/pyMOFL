...
from pyMOFL.functions import GriewankFunction
...
class TestGriewankFunction:
    def test_initialization(self):
        ...
        custom_init_bounds = Bounds(
            low=np.array([-10, -20, -30]),
            high=np.array([10, 20, 30]),
            mode=BoundModeEnum.INITIALIZATION,
        )
        custom_oper_bounds = Bounds(
            low=np.array([-5, -15, -25]), high=np.array([5, 15, 25]), mode=BoundModeEnum.OPERATIONAL
        )
        func = GriewankFunction(
            dimension=3,
