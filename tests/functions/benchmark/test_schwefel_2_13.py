...
from pyMOFL.functions import Schwefel_2_13


class TestSchwefel_2_13:
    def test_initialization(self):
        """Test initialization with default and custom bounds."""
        func = Schwefel_2_13(dimension=3)
        ...
        custom_init_bounds = Bounds(
            low=np.array([-1, -2, -3]), high=np.array([1, 2, 3]), mode=BoundModeEnum.INITIALIZATION
        )
        custom_oper_bounds = Bounds(
            low=np.array([-0.1, -0.2, -0.3]),
            high=np.array([0.1, 0.2, 0.3]),
    ...
    def test_evaluate_batch(self):
        ...
        alpha = rng.uniform(-np.pi, np.pi, dim)
        func = Schwefel_2_13(dimension=dim, a=a, b=b, alpha=alpha)
        X = np.array([alpha, alpha + 0.1, alpha - 0.2])
        expected = np.array([func.evaluate(X[0]), func.evaluate(X[1]), func.evaluate(X[2])])
    ...
    def test_dimension_validation(self):
        ...
        func = Schwefel_2_13(dimension=2)
