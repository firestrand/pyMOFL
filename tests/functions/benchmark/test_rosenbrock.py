...
from pyMOFL.functions import RosenbrockFunction
...
class TestRosenbrockFunction:
    ...
    def test_initialization(self):
        ...
        custom_oper_bounds = Bounds(
            low=np.array([-10, -5]), high=np.array([10, 5]), mode=BoundModeEnum.OPERATIONAL
        )
    ...
    def test_bounds_respect(self):
        ...
        custom_init_bounds = Bounds(
            low=np.array([-10, -10]), high=np.array([10, 10]), mode=BoundModeEnum.INITIALIZATION
        )
        custom_oper_bounds = Bounds(
            low=np.array([-10, -10]), high=np.array([10, 10]), mode=BoundModeEnum.OPERATIONAL
        )
        func = RosenbrockFunction(
            dimension=2,
            initialization_bounds=custom_init_bounds,
        ...
        edge_points = [np.array([-10, 0]), np.array([10, 0]), np.array([0, -10]), np.array([0, 10])]
