...
class TestStepFunction:
    ...
    def test_initialization(self):
        ...
        assert func.bias == 30.0
        
        # Test with custom bias
        func = StepFunction(bias=42.0)
    ...
    def test_evaluate_global_minimum(self):
        ...
        func = StepFunction(bias=30.0)
        
        # Test at origin (within optimal region [-0.5, 0.5))
        result = func.evaluate(np.zeros(10))
        assert result == 30.0
    ...
    def test_evaluate_stepped_values(self):
        ...
        for x_val, expected_step in test_cases:
            ...
            expected = expected_step**2 + func.bias
    ...
    def test_evaluate_multiple_steps(self):
        ...
        expected = np.sum(stepped**2) + func.bias
    ...
    def test_evaluate_batch(self):
        ...
        expected = np.array([
            ...
            np.sum(np.floor(batch[2] + 0.5)**2) + func.bias  # Complex case
        ])
    ...
    def test_non_negativity(self):
        """Test that function values are >= bias for points in the domain."""
        func = StepFunction(bias=30.0)
        ...
        # Check that all values are >= bias
        assert (values >= func.bias).all() 