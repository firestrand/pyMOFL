...
from pyMOFL.compositions import HybridFunction
...
class TestHybridFunction:
    ...
    def test_evaluate_multiple_components(self):
        ...
        expected = 0.5 * 2.0 + 0.5 * (
            20.0 + 2.0 - 10.0 * np.cos(2 * np.pi * 1.0) - 10.0 * np.cos(2 * np.pi * 1.0)
        )
    ...
    #     sphere = SphereFunction(dimension=2)
    #     shifted = ShiftWrapper(inner_function=sphere, shift=np.array([1.0, 1.0]))
    #     biased = BiasWrapper(inner_function=shifted, bias=5.0)
