"""Generic weighted-composition builder.
...
2. Building each component function from the registry (or recursively from nested
   composition configs).
3. Adding shared composition transforms: shift, scale, and rotation.
4. Appending normalization for weighted-composition behavior.

This keeps current CEC composition semantics while exposing the same generic
pipeline used for non-composite functions.
"""
...
def _is_valid_rotation_block(block: np.ndarray, *, dimension: int) -> bool:
    """Return whether a block looks numerically valid for runtime use.

    We intentionally keep this check lightweight and conservative to avoid
    rejecting legacy legacy-but-valid scaled transformations while still
    protecting against pathological data that can break composition behavior.
    """
...
class CompositionBuilder:
    """Build weighted composition functions from generic component configs."""
    ...
    def build(self, config: dict[str, Any], dimension: int) -> OptimizationFunction:
        """Build a weighted composition from its full config dict."""
        ...
        for i, comp_cfg in enumerate(functions_configs):
            ...
            component_input_transforms, component_output_transforms = (
