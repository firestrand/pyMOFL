...
class NetworkFunction(OptimizationFunction):
    ...
    def __init__(
        self, initialization_bounds: Bounds | None = None, operational_bounds: Bounds | None = None
    ):
        """
        ...
        if initialization_bounds is None:
            ...
            init_low = np.concatenate([np.zeros(binary_dim), np.zeros(cont_dim)])
            init_high = np.concatenate([np.ones(binary_dim), np.full(cont_dim, 20.0)])
            initialization_bounds = Bounds(
                ...
                qtype=np.array(
                    [
                        [QuantizationTypeEnum.INTEGER] * binary_dim
                        + [QuantizationTypeEnum.CONTINUOUS] * cont_dim
                    ]
                ).flatten(),
            )
        if operational_bounds is None:
            oper_low = np.concatenate([np.zeros(binary_dim), np.zeros(cont_dim)])
            oper_high = np.concatenate([np.ones(binary_dim), np.full(cont_dim, 20.0)])
