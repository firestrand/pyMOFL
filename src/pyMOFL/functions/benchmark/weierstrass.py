...
class WeierstrassFunction(OptimizationFunction):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._constant_term = dimension * np.sum(
            self._a_powers * np.cos(2 * np.pi * self._b_powers * 0.5)
        )

    def evaluate(self, x: np.ndarray) -> float:
        """Compute the Weierstrass function value."""
    ...
    def evaluate_batch(self, X: np.ndarray) -> np.ndarray:
        """Compute Weierstrass function for batch."""
