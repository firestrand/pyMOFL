...
The Exponential function is a simple unimodal function with exponential decay
from the origin.
...
@register("exponential")
class ExponentialFunction(OptimizationFunction):
    """
    ...
    def evaluate_batch(self, X: np.ndarray) -> np.ndarray:
        """Compute Exponential function for a batch of points."""
        X = self._validate_batch_input(X)
        return -np.exp(-0.5 * np.sum(X**2, axis=1))

    def get_global_minimum(self) -> tuple[np.ndarray, float]:
        """Get the global minimum of the Exponential function.
